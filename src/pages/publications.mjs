...
function citation(publication) {
  ...
  return `${publication.journal} ${publication.year}, ${publication.volume}${issue}, ${publication.pages}.`;
}
...
export function renderPublications() {
  ...
  <section class="section"><div class="container">
    ...
    <div class="publication-years">${years.map((year) => `<section class="publication-year" data-year="${escapeHtml(year)}"><h2>${escapeHtml(year)}</h2><ol>${publications.filter((publication) => publication.year === year).map(publicationItem).join('')}</ol></section>`).join('')}</div>
  </div></section>`;
}
